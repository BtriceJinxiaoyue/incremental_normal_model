function [Delta_State,u,phiu,varepsilon1,varepsilon2,Delta_W,bar_varepsilon1,bar_varepsilon2,dot_bar_varepsilon1,dot_bar_varepsilon2]=slidingcontrol3(t,x,L,B,fkx,fky,A_N,dotH,W,bar_varepsilon1,bar_varepsilon2,dot_bar_varepsilon1,dot_bar_varepsilon2,lbtf,dt1,signal_add)
...
tildef=zeros(2*Num,1);%formation keep distance
for i=1:Num
    for j=1:Num
        if A_N(i,j)>0
...
tildefx=zeros(2*Num,1);%formation keep distance
tildefv=zeros(2*Num,1);
if t>lbtf
    for i=1:Num
        for j=1:Num
            if A_N(i,j)>0
...
alpha11=[];
u_ld=[cos(t)*2.5;sin(t)*2.5];%leader's input
...
varepsilon2=x(2*Num+1:4*Num)-alpha1;%公式10
if t == [dt1]
